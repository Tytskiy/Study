...
\section{Задание №7}
Посмотрим как влияет предобработка данных, а конкретно параметры min\_df, max\_df.
В Таблице \ref{table:min_max_df} представлены зависимости времени, размерности,
 качества(BoW, TFIDF) от min\_df,max\_df.
\begin{table}[htb]
    \centering
...
Размерность пространства очень сильно увеличивается, если min\_df близко к нулю, но качество
в свою очередь несильно меняется. Как было упомянутно в начале исследования, обычно все 'плохие''
...
Max\_df неразумно ставить каким-либо, кроме максимального в данном случае т.к. мы можем потерять
важную информацию, а самые распространенные слова(стоп-слова) английского языка мы предварительно
убрали.

Интересно, что качество после обработки с помощью TFIDF стабильно хуже, чем BoW. \textbf{Лучший алгоритм
